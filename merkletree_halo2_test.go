...
func proofToRows(p *CircomProcessorProof) ([]Row, error) {
	...
	newLeafKey, err := NewNodeLeaf(p.NewKey, p.NewValue).Key()
	if err != nil {
		return nil, err
	}
	oldLeafKey, err := NewNodeLeaf(p.OldKey, p.OldKey).Key()
	if err != nil {
		return nil, err
	}
...
func generateTestData() error {
	...
	*/
	...
	fmt.Printf("p3 %+v\n", p3)
	p4, err := tree.Update(littleEndianIntBitsToBigInt(1, 0, 1, 0), big.NewInt(23))
	if err != nil {
		return err
	}
	fmt.Printf("p4 %+v\n", p4)
